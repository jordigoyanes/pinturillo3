...
export default {
  ...
  mounted() {
    ...
    this.socket.on("error_private_room", data => {
      ...
      switch (data.type) {
        case "not_found":
          this.errMessage = "Room not found, wrong room name or password";
          break;
        case "already_exists":
          this.errMessage = "Room already exists";
          break;
        default:
          this.errMessage = "Error joining or creating room";
          break;
      }
    });
  ...
  methods: {
    join_room() {
      if (this.room_password == "" || this.room_name == "") {
        this.showErr = true;
        this.errMessage = "Fill out all the inputs, please";
      } else {
        this.isLoading = true;
